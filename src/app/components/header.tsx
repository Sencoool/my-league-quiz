...
export default function Header() {
  ...
  return (
    <>
    <header className="w-full flex items-center justify-between px-4 sm:px-6 py-3 bg-[#1E293B] text-white shadow-md z-50">
      {/* Left: Logo */}
      ...
      <div className="flex items-center gap-2 sm:gap-4 shrink-0">
        ...
        {mounted && user?.isGuest && (
          <button
            ...
            className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-bold transition-colors shadow-sm cursor-pointer"
          >
            ...
            >
              ...
              <div className="relative shrink-0">
                <img 
                  ...
                  alt={user.username} 
                  className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border-2 transition-transform duration-200 group-hover:scale-105"
        ...
        <button 
          className="md:hidden p-2 text-zinc-300 hover:text-white bg-zinc-800/50 hover:bg-zinc-700/50 rounded-xl border border-white/10 transition-colors"
      ...
      <div 
        className={`fixed inset-0 z-[60] bg-black/80 transition-opacity duration-300 md:hidden ${showMobileMenu ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} 
