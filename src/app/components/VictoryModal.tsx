      ...
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 pointer-events-none">
        ...
        <div
          className={`pointer-events-auto relative w-full max-w-sm opacity-0 rounded-3xl
            bg-[#0f172a] border overflow-y-auto max-h-[92vh]
        ...
        >
        {/* Close */}
        <button onClick={onClose} className="absolute top-4 right-4 text-zinc-500 hover:text-white transition-colors z-[60] cursor-pointer">
            ...
            <div className="bg-zinc-800/50 border border-white/5 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center justify-center gap-1 text-xs">
                  <AnimatedNumberSpan value={targetDisplayScore} className="text-zinc-300 font-semibold font-mono" /> 
                  <span className="text-zinc-500 font-mono">PTS</span>
                </div>
                <div className="flex items-center gap-2">
                  {visualNextRankInfo ? (
                    <>
        ...
        {user?.isGuest && (
          <div className="w-full px-5 pb-3">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-900/40 to-indigo-900/40 border border-blue-500/30 p-5 group">
              <div className="absolute inset-0 bg-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className="relative z-10 flex flex-col items-center text-center gap-3">
                <div className="flex-1 w-full">
                  <h3 className="text-white font-bold text-base mb-1.5 flex items-center justify-center gap-1.5 font-sans tracking-wide">
                    <span className="text-yellow-400 text-lg drop-shadow-sm">⚠️</span> Don't lose your progress!
                  </h3>
                  <p className="text-blue-200/80 text-[13px] leading-relaxed font-sans px-2">
                    Guest accounts reset every 7 days. Sign in to save your streak!
                  </p>
                </div>
                <button
                  onClick={() => {
                  ...
                  className="shrink-0 relative overflow-hidden bg-white text-black font-bold text-sm px-6 py-2.5 rounded-xl flex items-center justify-center gap-2.5 hover:bg-gray-100 transition-colors shadow-[0_0_15px_rgba(255,255,255,0.2)] cursor-pointer font-sans mt-1"
                >
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {(["CLASSIC", "JIGSAW", "TRAITS", "MATCHER"] as const).map(m => {
              ...
              return isDone ? (
                ...
                <Link key={m} href={href} className="py-2.5 px-1 text-[11px] sm:text-xs font-semibold rounded-xl text-center bg-zinc-800/60 border border-white/10 text-zinc-300 hover:bg-blue-600 hover:border-blue-500 hover:text-white transition-all shadow-sm cursor-pointer">
                  {label}
                </Link>
              );
            })}
